...
library(sva)
totalRNA = as.data.frame(t(totalRNA[,1:(ncol(totalRNA)-1)]))
batch = c(rep("UCEC", length(ucecHis)),rep("OV", ncol(ovRNA)))
modCombat = model.matrix(~1, data = as.data.frame(batch))
totalRNAc = ComBat(dat = as.matrix(totalRNA), batch = batch,mod = modCombat, par.prior = T, prior.plots = F)
totalRNAc = as.data.frame(t(totalRNAc))

totalRNAc$anno = c(ucecHis, rep("OV", ncol(ovRNA)))
autoplot(prcomp(abs(totalRNAc[,1:(ncol(totalRNAc)-1)])), data = totalRNAc, colour = "anno", frame = TRUE, frame.type = 'norm')

table(colnames(ucecRNA) %in% rownames(totalCNV))
table(colnames(ovRNA) %in% rownames(totalCNV))
...
dim(ucecHi) 
colData(ucecHi)$subtype_histology #very few
dim(ucecGA)
